...
# รายชื่อ library ที่ต้องใช้ (ชื่อโมดูลสำหรับ import : ชื่อ package ใน pip)
REQUIRED_PACKAGES = {
    "PIL": "Pillow",
...
def install_and_import(module_name, package_name):
    try:
        ...
        print(f"กำลังติดตั้ง {package_name} อัตโนมัติ...")
        subprocess.check_call(
            [sys.executable, "-m", "pip", "install", package_name]
        )

# ตรวจสอบและติดตั้ง Package ที่ยังไม่มีในเครื่อง
...
def init_db():
    ...
    try:
        ...
        # บังคับสร้างคอลัมน์ engine_spec หากยังไม่มีในฐานข้อมูลเดิม
        try:
        ...
        is_connected_cloud = False
        messagebox.showerror(
            "Database Error", f"ไม่สามารถเชื่อมต่อฐานข้อมูลได้:\n{e}"
        )
...
def update_data(
    ...
    """ฟังก์ชันแก้ไข/อัปเดตข้อมูลลูกค้าเดิม"""
    conn = get_connection()
    cursor = conn.cursor()
    param = "%s" if USE_CLOUD else "?"
    ...
    fields = [
        f"date = {param}",
        f"customer_name = {param}",
        f"phone = {param}",
        f"car_brand = {param}",
        f"car_model = {param}",
        f"engine_spec = {param}",
        f"plate_number = {param}",
        f"ecu_type = {param}",
        f"remap_stage = {param}",
        f"price = {param}",
        f"note = {param}",
...
def clear_all_database_records():
    """ฟังก์ชันล้างข้อมูลในตารางทั้งหมด"""
    conn = get_connection()
    cursor = conn.cursor()
...
def reset_database_id_sequence():
    """ฟังก์ชัน Reset Auto Increment ID กลับไปเริ่มที่ 1"""
    conn = get_connection()
    cursor = conn.cursor()
    if USE_CLOUD:
...
def fetch_bin_file(customer_id):
    ...
    param = "%s" if USE_CLOUD else "?"
    cursor.execute(
        f"SELECT bin_filename, bin_data FROM customers WHERE id = {param}",
        (customer_id,),
    )
    row = cursor.fetchone()
    conn.close()
...
def fetch_xdf_file(customer_id):
    ...
    param = "%s" if USE_CLOUD else "?"
    cursor.execute(
        f"SELECT xdf_filename, xdf_data FROM customers WHERE id = {param}",
        (customer_id,),
    )
    row = cursor.fetchone()
    conn.close()
...
root.mainloop()import importlib
import subprocess
import sys
